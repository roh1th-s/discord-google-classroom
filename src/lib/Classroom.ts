...
class Classroom implements IClassroom {
  ...
  async listAnnouncements(): Promise<ClassroomAPI.Schema$Announcement[]> {
    ...
    return allAnnouncements.sort((a, b) => timestamp(`${a.creationTime}`) - timestamp(`${b.creationTime}`));
  }

  async listCourseWorkMaterial(): Promise<ClassroomAPI.Schema$CourseWorkMaterial[]> {
    const classroom = google.classroom({ version: 'v1', auth: this.client });

    const allCourseWorkMaterial: ClassroomAPI.Schema$CourseWork[] = [];
    ...
    for await (const c of this.courses) {
      ...
      if (courseWorkMaterial) {
        allCourseWorkMaterial.push(...courseWorkMaterial);
      }
    }

    return allCourseWorkMaterial.sort((a, b) => timestamp(`${a.creationTime}`) - timestamp(`${b.creationTime}`));
  ...
  async listCourseWork(): Promise<ClassroomAPI.Schema$CourseWork[]> {
    ...
    return allCourseWork.sort((a, b) => timestamp(`${a.creationTime}`) - timestamp(`${b.creationTime}`));
  }
